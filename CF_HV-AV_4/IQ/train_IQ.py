...
def main(cfg: DictConfig):
    ...
    expert_memory_replay.load(str(Path(os.getcwd()).parent.parent.absolute())+'/train_cf_4.npy',
    ...
    train_set = np.load(str(Path(os.getcwd()).parent.parent.absolute())+'/train_cf_4.npy', allow_pickle=True)
    trainNum = len(train_set.item()['states'])
    print('Number of training samples:', trainNum)

    # load testing dataset
    test_set = np.load(str(Path(os.getcwd()).parent.parent.absolute())+'/test_cf_4.npy', allow_pickle=True)
    ...
    while True:
        ...
        if begin_learn:
            ...
            print(
                'Epoch {}\tValidation-Training\tSpacing RMSPE: {:.6f}\tSpeed RMSPE: {:.6f}\tValidation-Testing\tSpacing RMSPE: {:.6f}\tSpeed RMSPE: {:.6f}'.format(
                    epoch, temp1, temp2, temp3, temp4))
            f.write(
                'Epoch {}\tValidation-Training\tSpacing RMSPE: {:.6f}\tSpeed RMSPE: {:.6f}\tValidation-Testing\tSpacing RMSPE: {:.6f}\tSpeed RMSPE: {:.6f}'.format(
                    epoch, temp1, temp2, temp3, temp4))
            f.write("\n")
            ...
            # save the best model
            if len(min_loss) == 0:
                min_loss.append(vali1_spacing_rmspe[-1]+vali2_spacing_rmspe[-1])
            else:
                if min_loss[-1] < vali1_spacing_rmspe[-1]+vali2_spacing_rmspe[-1]:
                    pass
                else:
                    min_loss[-1] = vali1_spacing_rmspe[-1]+vali2_spacing_rmspe[-1]
