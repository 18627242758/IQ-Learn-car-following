...
def evaluate(trainNum, train_set, actor, env, num_episodes=1, vis=True):
    ...
    while len(total_returns) < num_episodes:
        ...
        init_data = np.array(train_set.item()['states'][car_fol_id])
        state = env.reset(init_data)
        done = False

        with eval_mode(actor):
            while not done:
                action = actor.choose_action(np.array(state).reshape(1, -1), sample=False)
